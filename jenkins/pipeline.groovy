      ...
      stage('Deploy') {
         steps {
            ...
            script {
                ...
                sh("cd tf-deploy && terraform apply -auto-approve -var 'ami_id=${ami_id}' -var 'instance_type=${instance_type}' -var 'vpc_id=${vpc_id}' -var 'subnet_id=${subnet_id}' -var 'ec2_key_name=${ec2_key_name}' -var 'scanner_sg_id=${scanner_sg_id}'>/tmp/${env.BUILD_NUMBER}_tf_deploy.log")
            }
         }
      }
      stage('VM Scan') {
